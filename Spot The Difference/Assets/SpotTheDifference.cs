...
public class SpotTheDifference : MonoBehaviour
{
    ...
    void Start()
	{
		...
		_jon = UnityEngine.Random.Range(0,4);
		while (abcefgh[hgnjmhgnjmgfnb] == _jon)
		{
			_jon = UnityEngine.Random.Range(0,4);
		}
		Debug.LogFormat("[Spot The Difference #{0}] The faulty sphere is {1} (Starting from the bottom row, then the bottom left sphere according to the image and going in geometric order starting from 1). The color of the sphere is {2}.", moduleId, hgnjmhgnjmgfnb+1, logcolor[_jon]);
		WeedChungi[hgnjmhgnjmgfnb].GetComponent<MeshRenderer>().material = Colors[_jon];
